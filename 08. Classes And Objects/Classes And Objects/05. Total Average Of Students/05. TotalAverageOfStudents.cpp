...
double TotalAverageOfAllStudents(std::list<Student> students)
{
	double total = 0;

	for (Student student : students)
		total += student.totalAverage;

	return floor(total / students.size());
}
...
int main()
{
	...
	double totalAverageOfAllStudents = TotalAverageOfAllStudents(students);
	std::cout << totalAverageOfAllStudents;
}